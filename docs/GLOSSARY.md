...
A language-independent unit of repository knowledge extracted from a syntax tree. Examples include declarations, definitions, imports, exports, and package metadata. Repository facts are objective — they describe what exists in the repository without interpretation. Represented concretely by the `RepositoryFacts` domain artifact — an immutable collection of all extracted entities with evidence and deterministic ordering by `EntityId`.
...
An immutable, complete extracted semantic model for a repository. Owns all extracted entities with deterministic ordering (sorted by `EntityId`) and evidence attached to every entity. Produced by the Fact Extraction subsystem (Stage 3). Contains partitioned collections of modules, functions, structs, enums, traits, impl blocks, type aliases, constants, statics, imports, and exports.

## EntityId

A stable, deterministic identifier for every extracted entity. Generated by hashing five inputs — language, entity kind, file path, entity name, and byte offset. Identical source code always produces identical `EntityId` values, regardless of traversal order or processing history. EntityIds are reproducible across independent runs and stable for unchanged entities.

## Evidence

A verifiable reference to a repository artifact that supports a node, relationship, or derived fact. In the Fact Extraction subsystem, every entity carries an `Evidence` struct containing the source file path, syntax node kind, byte range, start/end line and column, and language. Evidence is always a `path:line` (or range) location in the repository. Evidence is never produced by AI — it is always extracted deterministically.

## Extractor

A trait (interface) for language-specific fact extraction. Every extractor implements `language()` — the programming language it handles — and `extract()` — walks a syntax tree and produces an `ExtractionOutcome` containing entities with evidence. Extractors are deterministic, perform no filesystem I/O, and are registered in an ExtractorRegistry.

## ExtractorRegistry

An ordered collection of `Extractor` implementations with first-match dispatch by language. Follows the same registry pattern as `ParserRegistry`. Supports `register()`, `dispatch(&Language)`, and `iter()`. The default registry includes `RustExtractor`.

## ExtractionOrchestrator

Drives the Stage 3 fact extraction pipeline. Iterates parsed syntax trees from `SyntaxTreeInventory`, selects the matching extractor via `ExtractorRegistry::dispatch()`, executes extractors, and merges all entities into a single `RepositoryFacts`. Performs no filesystem I/O — extraction is a pure transformation over immutable inputs.
